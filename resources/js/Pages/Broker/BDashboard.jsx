...
const OwnerRow = ({ item, onUpdate }) => (
    <tr className="hover:bg-white/[0.02] text-[15px] transition-all duration-300 group border-b border-white/5 last:border-0">
        ...
        <td className="p-6 text-center">
            {item.media?.length > 0 ? (
                <button className="bg-accent-gold/10 p-2 rounded-lg border border-accent-gold/20 text-accent-gold">
                    <span className="material-symbols-outlined text-sm">
                        play_circle
                    </span>
                </button>
            ) : (
                <span className="text-white/10 text-[9px]">لا ميديا</span>
            )}
        </td>
        <td className="p-6 text-center text-[15px]">
...
>
   <div className="py-12 px-4 md:px-8">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                {/* --- القسم الأول: جدول البيانات الرئيسي --- */}
                {viewMode === "data" && (
                    <div className="animate-fade-in space-y-10">
                        {/* Header Section + Tabs + Main Table */}
                        {/* هنا يظهر فقط جدول البائعين والمستثمرين والزبائن */}
                    </div>
                )}

               {/* --- القسم الأول: جدول البيانات الرئيسي (يظهر في وضع data) --- */}
                    {viewMode === "data" && (
                        <div className="animate-fade-in space-y-10">
                            {/* كود الهيدر وجدول البيانات القديم يوضع هنا */}
                        </div>
                    )}
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-center gap-7 bg-[#0b1c2d]/50 p-6 rounded-[2.5rem] border border-white/10 backdrop-blur-xl">
                                <div className="flex items-center gap-6">
                                    <div className="bg-[#d4af37] p-5 rounded-2xl shadow-[0_0_20px_rgba(212,175,55,0.2)]">
                                        <span className="material-symbols-outlined text-[#0b1c2d] text-2xl font-black">admin_panel_settings</span>
                                    </div>
                                    <div>
                                        <h3 className="text-2xl font-black text-white italic">إدارة <span className="text-[#d4af37]">صلاحيات الفريق</span></h3>
                                        <p className="text-white/30 text-[9px] font-bold uppercase tracking-[0.2em] gap-3 py-2">Security Control Center</p>
                                    </div>
                                </div>
    ...
    <div className="relative w-full max-w-2xl bg-[#0b1c2d] border border-white/10 rounded-[2rem] shadow-2xl 
                    max-h-[90vh] flex flex-col overflow-hidden">
        
        {/* هيدر النافذة (ثابت) */}
        <div className="p-6 border-b border-white/5 flex justify-between items-center shrink-0">
            <h2 className="text-white font-black uppercase tracking-widest text-sm">إصدار وثيقة تعيين</h2>
            <button onClick={() => setIsAddBrokerOpen(false)} className="text-white/40 hover:text-white">
                <span className="material-symbols-outlined">close</span>
            </button>
        </div>

           <div className="overflow-y-auto custom-scrollbar flex-1">
            <form onSubmit={handleAddBroker} className="p-5 md:p-10 space-y-6" dir="rtl">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">   
        
           {/* الاسم */}
                  
                    <div className="space-y-2">
                        <label className="text-white/40 text-[10px] font-black mr-2 uppercase">الاسم الكامل</label>
                        <input type="text" placeholder="أدخل اسم الوسيط" className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:border-[#d4af37] outline-none transition-all"
                            value={addBrokerForm.data.name} onChange={e => addBrokerForm.setData('name', e.target.value)} required />
                        {addBrokerForm.errors.name && <p className="text-rose-500 text-[10px] mt-1 mr-2">{addBrokerForm.errors.name}</p>}
                    </div>

                    {/* البريد */}
                    <div className="space-y-2">
                        <label className="text-white/40 text-[10px] font-black mr-2 uppercase">البريد الإلكتروني</label>
                        <input type="email" placeholder="email@example.com" className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white text-left focus:border-[#d4af37] outline-none transition-all"
                            value={addBrokerForm.data.email} onChange={e => addBrokerForm.setData('email', e.target.value)} required />
                    </div>

                    {/* رمز الدولة + الهاتف */}
                    <div className="space-y-2">
                        <label className="text-white/40 text-[10px] font-black mr-2 uppercase">رقم الهاتف</label>
                        <div className="flex gap-2" dir="ltr">
                             <select
                                      type="text" placeholder="+963" className="w-14 bg-white/5 border border-white/10 rounded-2xl px-2 py-3 text-white text-center focus:border-[#d4af37] outline-none"
                                value={addBrokerForm.data.country_code} onChange={e => addBrokerForm.setData('country_code', e.target.value)}
                                    >
                                        {countryCodes.map((c) => (
                                            <option
                                                key={c.code}
                            ...
                            <input type="text" placeholder="9xxxxxxxx" className=" w-48.5 flex-3 bg-white/5 border border-white/10 rounded-2xl px-4 py-4 text-white focus:border-[#d4af37] outline-none"
                                value={addBrokerForm.data.phone} onChange={e => addBrokerForm.setData('phone', e.target.value)} required />
                        </div>
                    </div>

                    {/* كلمة المرور */}
                    <div className="space-y-2">
                        <label className="text-white/40 text-[10px] font-black mr-2 uppercase">كلمة المرور</label>
                        <input type="password" placeholder="••••••••" className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white text-left focus:border-[#d4af37] outline-none transition-all"
                            value={addBrokerForm.data.password} onChange={e => addBrokerForm.setData('password', e.target.value)} required />
                    </div>
                </div>

              <div className="space-y-3">
        <label className="text-[#d4af37] text-[10px] font-black uppercase tracking-widest mr-2 block">امتيازات الوصول السيادية:</label>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4 bg-white/5 p-3 md:p-5 rounded-[1.5rem] md:rounded-[2rem] border border-white/10">
            {['owners', 'investors', 'customers'].map((f) => (
                <button 
                    key={f} 
                    type="button" 
                    onClick={() => addBrokerForm.setData(`can_view_${f}`, !addBrokerForm.data[`can_view_${f}`])}
                ...
                >
                    <span className={`material-symbols-outlined text-xl md:text-2xl ${addBrokerForm.data[`can_view_${f}`] ? 'text-[#d4af37]' : 'text-white/20'}`}>
                        {addBrokerForm.data[`can_view_${f}`] ? 'verified_user' : 'shield_lock'}
                    </span>
                    <span className="text-[9px] md:text-[10px] text-white font-black uppercase tracking-tighter">
                        {f === 'owners' ? 'بائعين' : f === 'investors' ? 'مستثمرين' : 'زبائن'}
                    </span>
                </button>
            ))}
        </div>
    ...
    >
        {addBrokerForm.processing ? "جاري منح الصلاحيات..." : "إصدار وثيقة التعيين"}
    </button>
</form>
 </div>
        </div>
            ...
            <div
                className="py-12 px-4 md:px-8 bg-[#0b1c2d] min-h-screen"
                dir="rtl"
            >
                <div className="max-w-7xl mx-auto space-y-8">
    ...
    {viewMode === "data" && (
        <div className="animate-fade-in space-y-10">
                    ...
                    <div
                        className="relative flex flex-col md:flex-row justify-between items-center border-b border-[#d4af37]/20 pb-10 mb-10 gap-8"
                        dir="rtl"
                    >
                        <div className="relative z-10 w-full max-w-full overflow-hidden">
    {/* التعديل: تقليل الـ gap والـ p في الجوال، وتصغير الحواف rounded */}
    <div className="flex items-center gap-3 md:gap-6 p-2.5 md:p-4 bg-[#d4af37] rounded-2xl md:rounded-[2rem] shadow-[0_10px_30px_rgba(212,175,55,0.3)] transition-all duration-500 hover:scale-[1.01]">
        
        {/* أيقونة العنوان - تصغير الـ p والـ text في الجوال */}
        <div className="bg-[#0b1c2d]/20 p-2 md:p-3 rounded-xl md:rounded-2xl shrink-0">
            <span className="material-symbols-outlined text-[#0b1c2d] text-xl md:text-3xl font-black">
                dashboard_customize
            </span>
        </div>

        <div className="pl-1 md:pl-4 flex-1 min-w-0">
            {/* العنوان: تصغير الخط من 3xl إلى xl في الجوال لضمان بقائه في سطر واحد */}
            <h2 className="text-lg md:text-3xl font-black text-[#0b1c2d] italic tracking-tighter leading-none truncate">
                إدارة{" "}
                <span className="underline decoration-[#0b1c2d]/20 underline-offset-4 md:underline-offset-8">
                    الصفقات الاستراتيجية
                </span>
            </h2>
            
            {/* النص الفرعي: تصغير التباعد tracking وتقليل الـ mt في الجوال */}
            <p className="text-[#0b1c2d]/60 text-[8px] md:text-[10px] font-black uppercase tracking-[0.1em] md:tracking-[0.3em] mt-1.5 md:mt-4 truncate">
                Sovereign Operations Center
            </p>
        </div>

        {/* العداد الوهمي HQ - يخفي النص الصغير في الجوال لبقاء المساحة */}
        <div className="bg-[#0b1c2d]/10 px-2 md:px-3 py-1 rounded-lg text-[#0b1c2d] text-[10px] md:text-xs font-black shrink-0">
            HQ
        </div>
    </div>
</div>
                        ...
                        <div className="flex flex-col items-center md:items-end gap-2 px-6">
                            <div className="flex items-center gap-3 bg-white/5 px-5 py-2.5 rounded-full border border-white/10">
                                ...
                                <p className="text-[#d4af37] text-[11px] font-black uppercase tracking-[0.4em]">
                                    Live Market Feed
                                </p>
                            </div>
                        </div>
...
<div className="bg-[#0b1c2d] border border-[#d4af37]/20 rounded-[2.5rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-md">
    <div className="overflow-x-auto">
        <table className="w-full text-right border-collapse min-w-[1000px]">
            <thead>
    <tr className="sticky top-0 z-10 bg-gradient-to-l from-[#d4af37] via-[#f2d472] to-[#d4af37] text-[#0b1c2d] uppercase text-[11px] font-black tracking-[0.2em] shadow-md">
        <th className="p-6 border-b border-white/10 text-right">الوسيط الرقمي</th>
        <th className="p-6 border-b border-white/10 text-center">بيانات الوصول (Email/Pass)</th>
        <th className="p-6 border-b border-white/10 text-center">بائعين</th>
        <th className="p-6 border-b border-white/10 text-center">مستثمرين</th>
        <th className="p-6 border-b border-white/10 text-center">زبائن</th>
        <th className="p-6 border-b border-white/10 text-center">إدارة</th>
    </tr>
</thead>
            <tbody className="text-white text-[15px] divide-y divide-white/5">
...
const BrokerPermissionRow = ({ broker }) => {
    ...
    const handleDelete = () => {
        if (confirm(`تحذير: هل أنت متأكد من حذف الوسيط ${broker.name} نهائياً؟`)) {
    ...
    return (
        <tr className="hover:bg-white/[0.02] transition-all group border-b border-white/5">
            {/* 1. الهوية */}
            <td className="p-6">
                <div className="font-black text-white text-[15px]">{broker.name}</div>
                <div className="text-[#d4af37]/40 text-[9px] italic tracking-widest uppercase">Agent ID: {broker.id}</div>
            </td>

            {/* 2. البريد وكلمة المرور - استخدمنا التحقق الاختياري ?. لضمان عدم الانهيار */}
            <td className="p-6 text-center">
                <div className="text-white font-bold text-[13px]">{broker.email || 'N/A'}</div>
                <div className="text-[#d4af37]/30 text-[9px] mt-1 font-mono truncate max-w-[150px] mx-auto" dir="ltr">
                    {broker.password ? `${broker.password.substring(0, 15)}...` : "NO_HASH_FOUND"}
                </div>
            </td>

            {/* 3. مربعات الصلاحيات - ربطناها بـ toggleField */}
            {['can_view_owners', 'can_view_investors', 'can_view_customers'].map((f) => (
                <td key={f} className="p-6 text-center">
                    ...
                    >
                        {data[f] && <span className="material-symbols-outlined text-[#0b1c2d] text-sm font-black">check</span>}
                    </button>
                </td>
            ))}
